...
_SYSTEM_PROMPT = (
    ...
    "batch.\n"
    "6. Vietnamese needs more syllables than Chinese to express the same "
    "meaning, which is a problem when this subtitle is fed into "
    "text-to-speech: a translation that's too long won't fit the "
    "original line's speaking duration, causing the generated voice to "
    "run long, overlap the next line, or get cut off. So: prefer the "
    "shortest natural phrasing that still reads as fluent, native "
    "Vietnamese — cut filler words, redundant connectors, and roundabout "
    "phrasing. Do not pad sentences or add explanatory words the source "
    "didn't need. Never sacrifice correctness or naturalness just to "
    "shave syllables, but when two equally natural phrasings exist, pick "
    "the shorter one.\n"
